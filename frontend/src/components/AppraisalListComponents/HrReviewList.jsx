    ...
    const fetchReviewAppraisalList= async () => {
      try {
        if (rolePermissions?.view) {
          const res = await api.get(`appraisals/review-appraisal-list/hr-review/`);
          console.log("HR Review Appraisal List:", res?.data);
          setReviewAppraisalList(Array.isArray(res.data) ? res.data : res.data ? [res.data] : [])
        }  
      } catch (error) {
        console.warn("Error fetching hr review appraisal List:", error);
