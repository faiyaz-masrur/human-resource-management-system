...
const CooReviewList = ({ rolePermissions, getReviewColor, getStatusColor, handleEditAppraisal }) => {
    ...
    const fetchReviewAppraisalList= async () => {
      try {
        if (rolePermissions?.view) {
          const res = await api.get(`appraisals/review-appraisal-list/coo-review/`);
          console.log("COO Review Appraisal List:", res?.data);
          setReviewAppraisalList(Array.isArray(res.data) ? res.data : res.data ? [res.data] : [])
        }  
      } catch (error) {
        console.warn("Error fetching coo review appraisal List:", error);
            ...
            {reviewAppraisalList.map((reviewAppraisal, index) => (
              <tr key={index}>
                ...
                <td>
                  <span 
                    style={{ 
                      color: getReviewColor(reviewAppraisal.coo_review ? 'Completed' : 'Pending'), 
                      fontWeight: 'bold' 
                    }}>
                    {reviewAppraisal.coo_review ? 'Completed' : 'Pending'}
                  </span>
                </td> 
