...
const CeoReviewList = ({ rolePermissions, getReviewColor, getStatusColor, handleEditAppraisal }) => {
    ...
    const fetchReviewAppraisalList= async () => {
      try {
        if (rolePermissions?.view) {
          const res = await api.get(`appraisals/review-appraisal-list/ceo-review/`);
          console.log("CEO Review Appraisal List:", res?.data);
          setReviewAppraisalList(Array.isArray(res.data) ? res.data : res.data ? [res.data] : [])
        }  
      } catch (error) {
        console.warn("Error fetching ceo review appraisal List:", error);
            ...
            {reviewAppraisalList.map((reviewAppraisal, index) => (
              <tr key={index}>
                ...
                <td>
                  <span 
                    style={{ 
                      color: getReviewColor(reviewAppraisal.ceo_review ? 'Completed' : 'Pending'), 
                      fontWeight: 'bold' 
                    }}>
                    {reviewAppraisal.ceo_review ? 'Completed' : 'Pending'}
                  </span>
                </td> 
