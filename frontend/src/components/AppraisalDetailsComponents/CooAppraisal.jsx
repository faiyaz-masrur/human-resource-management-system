...
const CooAppraisal = ({ appraisalId }) => {
  // State to hold all appraisal data (employee, manager, HR, CoO)
  const [appraisalData, setAppraisalData] = useState(null);
  // State for CoO-specific form data
  ...
  useEffect(() => {
    const fetchAppraisalData = async () => {
      ...
      try {
        const response = await axios.get(`/api/appraisals/coo-review/${appraisalId}/`);
        setAppraisalData(response.data);
        // Initialize form data with existing CoO data if it exists
        if (response.data.coo_review) {
          const cooReview = response.data.coo_review;
          setFormData({
            coo_remarks: cooReview.coo_remarks || '',
            final_decision: cooReview.final_decision || '',
            decision_remarks: cooReview.decision_remarks || '',
          });
        }
        setLoading(false);
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`/api/appraisals/coo-final-review/${appraisalId}/`, formData);
  ...
  const { employee_appraisal, manager_review, hr_review } = appraisalData;
  ...
  return (
    <form className="appraisal-form-container" onSubmit={handleSubmit}>
      ...
      {/* Employee, Manager, and HR Review Data */}
      ...
      <div className="form-section">
        <div className="form-header">
          <label htmlFor="coo-remarks" className="section-title">
            Remarks
          </label>
