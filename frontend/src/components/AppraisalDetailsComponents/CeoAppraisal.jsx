...
const CeoAppraisal = ({ appraisalId }) => {
  // State to hold all appraisal data (employee, manager, HR, COO, CEO)
  const [appraisalData, setAppraisalData] = useState(null);
  // State for CEO-specific form data
  ...
  useEffect(() => {
    const fetchAppraisalData = async () => {
      ...
      try {
        const response = await axios.get(`/api/appraisals/ceo-review/${appraisalId}/`);
        setAppraisalData(response.data);
        // Initialize form data with existing CEO data if it exists
        if (response.data.ceo_review) {
          const ceoReview = response.data.ceo_review;
          setFormData({
            ceo_remarks: ceoReview.ceo_remarks || '',
            final_decision: ceoReview.final_decision || '',
            decision_remarks: ceoReview.decision_remarks || '',
          });
        }
        setLoading(false);
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`/api/appraisals/ceo-final-review/${appraisalId}/`, formData);
  ...
  const { employee_appraisal, manager_review, hr_review, coo_review } = appraisalData;
  ...
  return (
    <form className="appraisal-form-container" onSubmit={handleSubmit}>
      ...
      {/* Employee, Manager, HR, and COO Review Data */}
      <div className="form-section">
        <h2 className="section-title">Appraisal History</h2>
        {employee_appraisal && (
          <div className="appraisal-data-display mb-6">
            <h3 className="text-lg font-bold">Employee's Self-Appraisal:</h3>
        ...
        {manager_review && (
          <div className="appraisal-data-display mb-6">
            <h3 className="text-lg font-bold">Manager's Review:</h3>
        ...
        {hr_review && (
          <div className="appraisal-data-display mb-6">
            <h3 className="text-lg font-bold">HR's Review:</h3>
            <p><strong>HR Remarks:</strong> {hr_review.hr_remarks}</p>
            <p><strong>Proposed Basic:</strong> {hr_review.proposed_basic}</p>
            <p><strong>Proposed Gross:</strong> {hr_review.proposed_gross}</p>
            <p><strong>Final Decision:</strong> {hr_review.final_decision.replace(/_/g, ' ')}</p>
            <p><strong>Decision Remarks:</strong> {hr_review.decision_remarks}</p>
          </div>
        )}
        {coo_review && (
          <div className="appraisal-data-display">
            <h3 className="text-lg font-bold">COO's Review:</h3>
            <p><strong>COO Remarks:</strong> {coo_review.coo_remarks}</p>
            <p><strong>Final Decision:</strong> {coo_review.final_decision.replace(/_/g, ' ')}</p>
            <p><strong>Decision Remarks:</strong> {coo_review.decision_remarks}</p>
          </div>
        )}
      </div>
      ...
      <div className="form-section">
        <div className="form-header">
          <label htmlFor="ceo-remarks" className="section-title">
            Remarks
          </label>
