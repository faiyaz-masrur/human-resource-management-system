...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for timer in EmployeeAppraisalTimer.objects.filter(self_appraisal_start=today):
            for track in EmployeeAppraisalTrack.objects.filter(
                ...
                Notification.objects.create(
                    ...
                    message=f"Your appraisal period starts today and will end on {timer.self_appraisal_end}."
                )
                send_mail(
                    "Appraisal Started",
                    f"Your appraisal period starts today and will end on {timer.self_appraisal_end}. "
                    f"Please submit your appraisal before the end date.",
                    settings.DEFAULT_FROM_EMAIL,
                    [track.employee.email]
                )

        # --- Reporting manager review start ---
        ...
        for timer in EmployeeAppraisalTimer.objects.filter(self_appraisal_remind=today):
            for track in EmployeeAppraisalTrack.objects.filter(
                appraisal__appraisal_timer=timer, status="not_started"
            ):
                Notification.objects.create(
                    ...
                    message=f"Please submit your appraisal before {timer.self_appraisal_end}."
                )
                send_mail(
                    "Reminder: Submit Appraisal",
                    f"Please submit your appraisal before {timer.self_appraisal_end}.",
                    settings.DEFAULT_FROM_EMAIL,
                    [track.employee.email]
                )

        # --- Reporting manager review reminder (pending reviews) ---
        ...
        self.stdout.write(self.style.SUCCESS('Appraisal notifications sent successfully.'))
